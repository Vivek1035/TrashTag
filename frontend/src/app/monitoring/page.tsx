...
export default function MonitoringDashboardPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-16">
      {/* Header Bar */}
      <header className="sticky top-0 z-30 bg-slate-900/90 backdrop-blur-md border-b border-slate-800 px-4 py-4 shadow-md">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              30, 60, and 90-day post-transformation site inspection protocol & sustainability tracking
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-cyan-500/10 text-cyan-400 border border-cyan-500/30">
              No Auto-Clean Without Inspection Record
            </span>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 mt-6 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          <div className="bg-slate-900/90 border border-slate-800 p-4 rounded-2xl space-y-1">
            <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400 block">Due</span>
          ...
          <div className="bg-slate-900/90 border border-slate-800 p-4 rounded-2xl space-y-1">
            <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400 block">Overdue</span>
          ...
          <div className="bg-slate-900/90 border border-slate-800 p-4 rounded-2xl space-y-1">
            <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400 block">Sustained</span>
          ...
          <div className="bg-slate-900/90 border border-slate-800 p-4 rounded-2xl space-y-1 col-span-2 sm:col-span-1">
            <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400 block">Reopened</span>
          ...
          <div className="flex items-center gap-1 bg-slate-950 p-1 rounded-xl border border-slate-800 text-xs">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-bold transition-colors ${
                selectedDayFilter === null ? 'bg-cyan-600 text-white' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              ALL DAYS
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-bold transition-colors ${
                selectedDayFilter === 30 ? 'bg-cyan-600 text-white' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              30 DAYS
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-bold transition-colors ${
                selectedDayFilter === 60 ? 'bg-cyan-600 text-white' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              60 DAYS
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-bold transition-colors ${
                selectedDayFilter === 90 ? 'bg-cyan-600 text-white' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              90 DAYS
            </button>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3">
            ...
            <form onSubmit={handleSearchSubmit} className="relative w-full sm:w-64">
              ...
              <input
                ...
                className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-9 pr-3 py-2 text-xs text-slate-200 focus:outline-none focus:ring-2 focus:ring-cyan-500/50"
              />
            </form>
          </div>
        </div>
        ...
        {loading ? (
          <div className="py-12 flex flex-col items-center justify-center gap-3">
            ...
            <p className="text-xs text-slate-400">Loading monitoring checkpoints...</p>
          </div>
        ) : checkpoints.length === 0 ? (
          <div className="bg-slate-900/60 border border-slate-800 border-dashed rounded-2xl p-12 text-center text-slate-400 space-y-2">
            <Calendar className="w-10 h-10 text-slate-600 mx-auto" />
            <h3 className="text-sm font-bold text-slate-200">No Monitoring Checkpoints Found</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {checkpoints.map((cp) => (
              <div
                key={cp.id}
                className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 space-y-4 shadow-lg flex flex-col justify-between hover:border-slate-700 transition-all"
              >
                <div className="space-y-3">
                  ...
                  {/* Title & Address */}
                  <div>
                    <h3 className="text-sm font-bold text-slate-100 line-clamp-1">{cp.trashTagTitle}</h3>
                    {cp.address && <p className="text-xs text-slate-400 truncate mt-0.5">{cp.address}</p>}
                  </div>
                  ...
                  <div className="bg-slate-950 p-3 rounded-xl border border-slate-800/80 space-y-1.5 text-xs text-slate-300">
                    ...
                    {cp.wasteDetected !== undefined && cp.wasteDetected !== null && (
                      <div className="flex items-center justify-between pt-1 border-t border-slate-800">
                        <span className="text-slate-400">Waste Return Status:</span>
                        <span
                          className={`font-bold ${
                  ...
                  {/* Evidence Thumbnail */}
                  {cp.image && (
                    <div className="relative rounded-xl overflow-hidden h-36 bg-slate-950 border border-slate-800">
                      <img
                        src={cp.image}
                        alt={`Inspection Evidence - ${cp.tagCode}`}
                        className="w-full h-full object-cover"
                      />
                      <div className="absolute top-2 left-2 bg-black/70 backdrop-blur-sm text-[10px] text-slate-200 px-2 py-0.5 rounded font-mono">
                        Inspection Evidence
                      </div>
                    </div>
                  )}
                  ...
                  {/* Notes */}
                  {cp.notes && (
                    <p className="text-xs text-slate-400 line-clamp-2 italic bg-slate-950/40 p-2.5 rounded-lg border border-slate-800/60">
                      "{cp.notes}"
                    </p>
                  )}
                </div>
              ...
              <form onSubmit={handleSubmitInspection} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-300 block">
                    Inspection Evidence Photo URL (Optional)
                  </label>
                  <input
                    type="url"
                    value={evidenceImageUrl}
                    onChange={(e) => setEvidenceImageUrl(e.target.value)}
                    placeholder="https://images.unsplash.com/inspection-photo.jpg"
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-slate-200 focus:outline-none focus:ring-2 focus:ring-cyan-500/50"
                  />
                </div>
