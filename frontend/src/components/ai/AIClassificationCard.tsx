...
export const AIClassificationCard: React.FC<AIClassificationCardProps> = ({
  ...
  const handleClassify = async () => {
    if (!token) {
      setError('Log in to run AI evidence classification.');
      return;
    }
    setLoading(true);
    setError(null);
    try {
      const data = await classifyTrashTagApi(tag.id, token);
  ...
  const handleApplyOverride = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!token) {
      setError('Log in to override classification.');
      return;
    }
    ...
    <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 space-y-4 shadow-xl">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-3.5">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400">Automated visual analysis of evidence photo</p>
          </div>
        </div>
      ...
      {classification ? (
        <div className="space-y-4 pt-1">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex flex-wrap gap-1.5">
              {classification.detectedObjects.map((obj, i) => (
                <span
                  key={i}
                  className="px-2.5 py-1 rounded-lg bg-slate-950 text-slate-300 border border-slate-800 text-xs font-medium font-mono"
                >
                  #{obj}
                </span>
              ))}
          ...
          <div className="pt-2 flex justify-end">
            <button
              ...
              className="px-3.5 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-bold rounded-lg transition-colors flex items-center gap-1.5 border border-slate-700"
            >
        ...
        <div className="bg-slate-950/50 rounded-xl border border-slate-800 border-dashed p-6 text-center text-slate-400 text-xs space-y-2">
          ...
          <p className="font-medium text-slate-300">Click "Analyze Image" to run AI visual classification</p>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-md w-full p-6 space-y-5 shadow-2xl">
            ...
            <form onSubmit={handleApplyOverride} className="space-y-4">
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-300 block">Waste Type</label>
                <select
                  value={selectedWasteType}
                  onChange={(e) => setSelectedWasteType(e.target.value as WasteType)}
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-xs text-slate-200 focus:outline-none focus:ring-2 focus:ring-amber-500/50"
                >
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-300 block">Severity Rating</label>
                <select
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-300 block">Override Reason / Notes</label>
                <textarea
                  value={overrideNotes}
