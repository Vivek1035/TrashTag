  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-2xl transition-all duration-300">
      ...
      <div className="relative h-44 w-full bg-slate-800 overflow-hidden">
        ...
        <div className="absolute top-3 left-3 bg-slate-950/80 backdrop-blur-md px-2.5 py-1 rounded-md text-xs font-mono font-bold text-emerald-400 border border-emerald-500/40 shadow-lg">
          #{tag.tagCode}
        </div>
        ...
        {onClose && (
          <button
            onClick={onClose}
            className="absolute top-3 right-3 bg-slate-950/70 hover:bg-slate-950 text-slate-300 hover:text-white w-7 h-7 rounded-full flex items-center justify-center text-xs backdrop-blur-md border border-slate-700/50 transition-colors"
          >
            ✕
          </button>
        )}
      ...
      <div className="p-4 space-y-3.5">
        ...
        <div className="flex items-center justify-between text-[11px] text-slate-400 border-t border-slate-800/80 pt-2.5">
          <span className="flex items-center gap-1">
            ...
            Reported by <strong className="text-slate-300 font-medium">{tag.reporterName || 'Anonymous'}</strong>
          </span>
