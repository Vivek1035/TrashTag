    ...
    <div className="bg-white/90 dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 space-y-4 shadow-xl">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3.5">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-600 dark:text-slate-400">3-Stage evolution from illegal dump to revitalized community space</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1 bg-slate-50 dark:bg-slate-950 p-1 rounded-lg border border-slate-200 dark:border-slate-800 text-[11px]">
          <button
            ...
            className={`px-2.5 py-1 rounded font-semibold transition-colors ${
              activeStage === 'all' ? 'bg-purple-600 text-white' : 'text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:text-slate-200'
            }`}
          >
            3-Stage Timeline
          </button>
          <button
            ...
            className={`px-2.5 py-1 rounded font-semibold transition-colors ${
              activeStage === 'before' ? 'bg-amber-600 text-white' : 'text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:text-slate-200'
            }`}
          >
            Before Only
          </button>
          <button
            ...
            className={`px-2.5 py-1 rounded font-semibold transition-colors ${
              activeStage === 'after' ? 'bg-emerald-600 text-white' : 'text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:text-slate-200'
            }`}
          >
            After Only
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        {(activeStage === 'all' || activeStage === 'before') && (
          <div className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden space-y-3 p-3 flex flex-col justify-between">
            <div className="relative rounded-lg overflow-hidden h-48 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
              {beforeImageUrl ? (
                <img
                  src={beforeImageUrl}
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-600 dark:text-slate-400 mt-0.5">Initial report submitted by community reporter.</p>
            </div>
          </div>
        )}
        ...
        {activeStage === 'all' && (
          <div className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-4 flex flex-col justify-between space-y-3">
            <div className="flex-1 flex flex-col items-center justify-center text-center p-4 bg-slate-100/80 dark:bg-slate-900/60 rounded-lg border border-slate-200 dark:border-slate-800/80 space-y-2">
              <ShieldCheck className="w-10 h-10 text-emerald-400" />
              <div className="space-y-0.5">
                <span className="text-2xl font-extrabold text-emerald-400 font-mono">
                  {recoveredWeightKg || 0} kg
                </span>
                <span className="text-xs font-semibold text-slate-700 dark:text-slate-300 block">Recovered Waste Swept</span>
              </div>
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-600 dark:text-slate-400 mt-0.5">Volunteers removed hazardous materials.</p>
            </div>
          </div>
        )}
            ...
            <div className="relative rounded-lg overflow-hidden h-48 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
              {afterImageUrl ? (
                ...
                <div className="w-full h-full flex flex-col items-center justify-center text-slate-600 dark:text-slate-400 gap-2 p-4 text-center">
                  ...
                  <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">Transformation Image Pending</span>
